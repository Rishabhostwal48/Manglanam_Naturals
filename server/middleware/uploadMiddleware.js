...
if (!fs.existsSync(uploadsDir)) {
  ...
  console.log('Creating uploads directory at:', uploadsDir); // For debugging
}
...
const fileFilter = (req, file, cb) => {
  ...
  if (file.fieldname === 'video') {
    ...
    const videoExtname = videoFiletypes.test(path.extname(file.originalname).toLowerCase());
    const videoMimetype = /video\/mp4|video\/webm|video\/quicktime|video\/mov|video\/x-quicktime/.test(file.mimetype);
    ...
    if (videoExtname && videoMimetype) {
      ...
      cb(new Error('Videos only! Please upload a video file (mp4, webm, quicktime)'));
    }
  } else {
    // For image uploads
